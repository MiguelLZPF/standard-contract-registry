...
struct Deployment {
  ...
  address logic; // Address of the logic contract.
  bytes32 logicCodeHash; // Hash of the bytecode for the deployment.
  // bytes32 logicDeployTxHash;
  bytes32 name; // Name of the deployment.
  address proxy; // Address of the proxy contract.
...
contract Configuration is Script {
  ...
  function storeDeployment(Deployment calldata deployment) external {
    ...
    vm.serializeAddress(toBeDeployment, "proxyAddr", deployment.proxy);
    vm.serializeAddress(toBeDeployment, "logicAddr", deployment.logic);
