...
interface ICodeTrust {
   ...
   * @dev it checks if the address is a contract address or a EOA comparing the msg.sender and tx.origin to force having expiration or not
   * @param trustedCode the address of the contract address to be trusted
   * @param expiration (optional) [1] datetime to trust this contract code. Mandatory if this call comes from a EOA. If sender is another contract, can be 1 meaning no expiration
   */
  function trustCodeAt(address trustedCode, uint256 expiration) external;
